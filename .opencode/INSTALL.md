...
- 安装仓库：`https://github.com/LycanW/program-notebook.git`
- 默认安装目录：`~/.config/program-notebook`
- 注册插件入口：`~/.config/program-notebook/.opencode/plugins/program-notebook.ts`
...
2. 如果 `~/.config/program-notebook/.git` 存在，进入该目录并拉取最新 `main`。
3. 如果目录不存在，克隆仓库到 `~/.config/program-notebook`。
...
7. 在配置的 `plugin` 数组中追加绝对 `file://` URL：`file:///home/USER/.config/program-notebook/.opencode/plugins/program-notebook.ts`。
...
```jsonc
{
  ...
  "plugin": [
    "file:///home/USER/.config/program-notebook/.opencode/plugins/program-notebook.ts"
  ]
}
```
